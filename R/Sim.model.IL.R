...
# Function to compute power for ESM studies as a function of the number of participants

Sim.model.ESM = function(Model,N,N.0,N.1,n.days,n.beeps, 
...
if (length(alpha) == 0) {stop('The Type I error must be positive')}
if (alpha == 0) {stop('The Type I error must be positive')}
if (length(n.days) == 0) {stop('Set the number of days')}
if (length(n.beeps) == 0) {stop('Set the number of beeps per day')}
if (length(sigma) == 0) {stop('Set the value of the standard error of the level-1 residual')}
if (sigma <= 0) {stop('The variance of the level-1 standard error must be positive')}
# if (T<20) {stop('The total number of occasions must be equal or larger than 20')}
...
#clusterExport(clust, "mvrnorm")
#clusterExport(clust, "lme")
#clusterExport(clust, "data")
#clusterExport(clust, "lmeControl")
#clusterExport(clust, "corAR1")
#clusterExport(clust, "Sim.Data.ESM")
#clusterExport(clust, "Fit.model.ESM")
#clusterExport(clust, "Summary.model.ESM")
#clusterExport(clust, "intervals")
#clusterExport(clust, "VarCorr")
#clusterExport(clust, c("Model","N","N.0","N.1","n.days","n.beeps", 
#"isX.center","Ylag.center","isW.center","alpha","R", 
#"b00","b01.Z","b01.W","b10","b11.Z","b11.W","sigma","rho","sigma.v0","sigma.v1","rho.v", 
#"mu.W","sigma.W","mu.X","mu.X0","mu.X1","sigma.X","sigma.X0","sigma.X1","Opt.Method","is.rho.zero"))

#fit = parLapply(clust,1:length(N.0), function(i)
fit = lapply(1:length(N.0), function(i)
Summary.model.ESM(Model, N, N.0[i], N.1[i], n.days, n.beeps, 
isX.center, Ylag.center, isW.center, alpha, 
b00, b01.Z, b01.W, b10, b11.Z, b11.W, sigma, rho, sigma.v0, sigma.v1,rho.v, 
mu.W, sigma.W, mu.X, mu.X0, mu.X1, sigma.X, sigma.X0, sigma.X1,R,Opt.Method,is.rho.zero))
...
data = Sim.Data.ESM(Model,N,N.0[N.max], N.1[N.max], n.days, n.beeps,isX.center,Ylag.center,isW.center, 
b00, b01.Z, b01.W, b10, b11.Z, b11.W, sigma, rho, sigma.v0, sigma.v1, 
rho.v, mu.W, sigma.W, mu.X, mu.X0, mu.X1, sigma.X, sigma.X0, sigma.X1)
...
T = rep(1:(n.days*n.beeps),2)
data$Z = as.factor(data$Z)
Ni.05 = c(which(data$subjno==N0.Q05),which(data$subjno==N1.Q05))
Ni.5 = c(which(data$subjno==N0.Q5),which(data$subjno==N1.Q5))
Ni.95 = c(which(data$subjno==N0.Q95),which(data$subjno==N1.Q95))

data.ESM.Q05 = data.frame(cbind(data[Ni.05,],T=T))
data.ESM.Q5 = data.frame(cbind(data[Ni.5,],T=T))
data.ESM.Q95 = data.frame(cbind(data[Ni.95,],T=T))

Y.pool = rbind(data.ESM.Q05,data.ESM.Q5,data.ESM.Q95)$Y
Y.ESM.min = min(Y.pool)
Y.ESM.max = max(Y.pool)
}
...
#fit = parLapply(clust,1:length(N), function(i)
fit = lapply(1:length(N), function(i)
Summary.model.ESM(Model, N[i], N.0, N.1, n.days, n.beeps, 
isX.center, Ylag.center, isW.center, alpha, 
b00, b01.Z, b01.W, b10, b11.Z, b11.W, sigma, rho, sigma.v0, sigma.v1,rho.v, 
mu.W, sigma.W, mu.X, mu.X0, mu.X1, sigma.X, sigma.X0, sigma.X1,R,Opt.Method,is.rho.zero))
...
data = Sim.Data.ESM(Model,N[N.max],N.0, N.1, n.days, n.beeps,isX.center,Ylag.center,isW.center, 
b00, b01.Z, b01.W, b10, b11.Z, b11.W, sigma, rho, sigma.v0, sigma.v1, 
rho.v, mu.W, sigma.W, mu.X, mu.X0, mu.X1, sigma.X, sigma.X0, sigma.X1)
...
#clusterExport(clust, "intervals")
#clusterExport(clust, "VarCorr")
#clusterExport(clust, "lag.Y")
#clusterExport(clust, "shift")
#clusterExport(clust, c("Model","N","N.0","N.1","n.days","n.beeps", 
#"isX.center","Ylag.center","isW.center","alpha","R", 
#"b00","b01.Z","b01.W","b10","b11.Z","b11.W","sigma","rho","sigma.v0","sigma.v1","rho.v", 
#"mu.W","sigma.W","mu.X","mu.X0","mu.X1","sigma.X","sigma.X0","sigma.X1","Opt.Method","is.rho.zero"))
...
return(list(power.curve=power.curve,coef.sim=coef.sim,b.sim=b.sim,
data.ESM.Q05=data.ESM.Q05,data.ESM.Q5=data.ESM.Q5,data.ESM.Q95=data.ESM.Q95,
Y.ESM.min=Y.ESM.min,Y.ESM.max=Y.ESM.max,cov.sim=cov.sim))}
