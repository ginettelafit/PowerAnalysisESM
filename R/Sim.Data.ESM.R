...
# Function to simulate an ESM data set

Sim.Data.ESM = function(Model,N,N.0, N.1, n.days, n.beeps,isX.center,Ylag.center,isW.center, 
...
data.ESM = expand.grid(Beep=1:n.beeps,Day=1:n.days,Z=n.group)

# Create variable subjno
subjno = expand.grid(1:(n.beeps*n.days),1:N)[,2]
data.ESM = cbind(subjno,data.ESM)
...
E = rep(0,T*N)
for (i in 1:N){
E[which(data.ESM$subjno==i)] = arima.sim(n=T,AR.epsilon)*sigma*sqrt(1-rho^2)
}}
...
V = rep(0,T*N)
for (i in 1:N){V[which(data.ESM$subjno==i)] = V.i[i]}

B00 = rep(b00,nrow(data.ESM))
B01 = rep(b01.Z,nrow(data.ESM))

# Simulate Dependent Variable
Y = B00 + B01*Z + V + E

# Create a data frame
data = data.frame(cbind(data.ESM,Y))
}
...
# Create variables days, beeps per day and Z
data.ESM = expand.grid(Beep=1:n.beeps,Day=1:n.days,W=W.i)
...
# Create variables days, beeps per day and Z
data.ESM = expand.grid(Beep=1:n.beeps,Day=1:n.days,subjno=1:N)
...
V = matrix(0,T*N,2)
for (i in 1:N){
  V[which(data.ESM$subjno==i),1] = V.i[i,1]
  V[which(data.ESM$subjno==i),2] = V.i[i,2]
}
...
X = rep(0,N*T)
for (i in 1:N){
  X[which(data.ESM$subjno==i)] = rnorm(T,mean=mu.X,sd=sigma.X)
}

B00 = rep(b00,nrow(data.ESM))
B10 = rep(b10,nrow(data.ESM))

if (isX.center==TRUE){
# Mean centered time varying variable per-individual
for (i in 1:N){
  X[which(data.ESM$subjno==i)] = X[which(data.ESM$subjno==i)] - mean(X[which(data.ESM$subjno==i)])
}}
...
# Create a data frame
data = data.frame(cbind(data.ESM,Y,X)) 
}
...
X = rep(0,N*T)
for (i in 1:N){
  X[which(data.ESM$subjno==i)] = ifelse(Z[which(data.ESM$subjno==i)]==0,rnorm(T,mean=mu.X0,sd=sigma.X0),
...
Y[which(data.ESM$subjno==i)] = arima.sim(n=T,AR.epsilon)*sqrt(1-gamma.1[i]^2)*sigma + gamma.0[i]}
}

if (Ylag.center == FALSE){
for (i in 1:N){
AR.epsilon = list(order=c(1,0,0), ar=gamma.1[i], include.mean = FALSE)
Y[which(data.ESM$subjno==i)] = arima.sim(n=T,AR.epsilon)*sqrt(1-gamma.1[i]^2)*sigma + gamma.0[i]/(1-gamma.1[i])
}}
...
Y[which(data.ESM$subjno==i)] = arima.sim(n=T,AR.epsilon)*sqrt(1-gamma.1^2)*sigma + gamma.0
}

if (Ylag.center == FALSE){
AR.epsilon = list(order=c(1,0,0), ar=gamma.1, include.mean = FALSE)
Y[which(data.ESM$subjno==i)] = arima.sim(n=T,AR.epsilon)*sqrt(1-gamma.1^2)*sigma + gamma.0/(1-gamma.1)
}}
...
# End of function ---> Return simulated ESM data set
